...
@Component({
              ...
              <tbody>
                ...
                @else {
                  @for (derog of filteredDerogations(); track derog.id) {
                    <!-- LIGNE PRINCIPALE -->
                    <tr class="clickable" [class.expanded-row]="expandedId() === derog.id" (click)="toggleExpand(derog.id)">
                      <td>
                        <div class="user-cell">
                      ...
                      <td><i class="bi bi-chevron-down expand-icon" [class.rotated]="expandedId() === derog.id"></i></td>
                    </tr>
                        ...
                        <td colspan="6">
                          <div class="details-panel">
                            ...
                            <div class="workflow-container">
                              ...
                              <div class="step-line" [class.active]="getWorkflowStep(derog) >= 2"></div>

                              <div class="step" [ngClass]="getStepClass(derog, 2)">
                                <div class="step-circle">
                                  @if(getWorkflowStep(derog) >= 3) { <i class="bi bi-check-lg"></i> }
                                  @else { <i class="bi bi-person-check"></i> }
                                </div>
                                <span class="step-label">Directeur</span>
                              </div>

                              <div class="step-line" [class.active]="getWorkflowStep(derog) >= 3"></div>

                              <div class="step" [ngClass]="getStepClass(derog, 3)">
                                <div class="step-circle">
                                  @if(getWorkflowStep(derog) > 3) { <i class="bi bi-check-lg"></i> }
                              ...
                              <div class="step-line" [class.active]="getWorkflowStep(derog) >= 4"></div>

                              <div class="step" [ngClass]="getStepClass(derog, 4)">
                                <div class="step-circle">
                                  @if(getWorkflowStep(derog) >= 4) { <i class="bi bi-check-lg"></i> }
                            ...
                            <div class="detail-grid">
                              <div class="detail-item">
                                ...
                                <span class="value">{{ derog.doctorantPrenom }} {{ derog.doctorantNom }}</span>
                              </div>
                              <div class="detail-item">
                                <label>Directeur</label>
                                <span class="value">{{ getDirecteurName(derog) }}</span>
                              </div>
                              <div class="detail-item">
                                <label>Type de dérogation</label>
                                <span class="value">{{ formatType(derog.typeDerogation) }}</span>
                              </div>
                              <div class="detail-item">
                                <label>Date de demande</label>
                                <span class="value">{{ derog.dateDemande | date:'dd/MM/yyyy' }}</span>
                              </div>
                              <div class="detail-item full-width">
                                <label>Motif de la demande</label>
                                <div class="motif-box">{{ derog.motif }}</div>
                              </div>
                              @if (derog.commentaireDirecteur) {
                                <div class="detail-item full-width">
                                  <label>Avis du Directeur</label>
                                  <div class="comment-box director">{{ derog.commentaireDirecteur }}</div>
                                </div>
                              }
                            </div>
                                ...
                                <div class="admin-action-box">
                                  ...
                                  @if (!isRejecting()) {
                                    ...
                                    <div class="action-buttons">
                                      ...
                                      <button class="btn-validate" (click)="accepter(derog.id, $event)">Accepter & Prolonger</button>
                                    </div>
                                  } @else {
                                    <div class="decision-form">
                                      ...
                                      <div class="d-flex gap-2 justify-content-end">
                                        <button class="btn-cancel" (click)="cancelRefusal($event)">Annuler</button>
                                        <button class="btn-confirm-refuse" [disabled]="!refusalReason.trim()" (click)="confirmRefusal(derog.id, $event)">Confirmer le refus</button>
                                      </div>
                                    </div>
                                  }
                                </div>
                            ...
                            <!-- INFO: EN ATTENTE DIRECTEUR -->
                            @if (derog.statut === 'EN_ATTENTE_DIRECTEUR' || derog.statut === 'EN_ATTENTE') {
                            ...
                            <!-- INFO: APPROUVÉE -->
                            @if (derog.statut === 'APPROUVEE') {
                              <div class="info-box success mt-3">
                                <i class="bi bi-check-circle-fill text-success"></i>
                                <div>
                                  <strong>Dérogation approuvée</strong>
                                  <p class="mb-0 mt-1 small">Le doctorant a été autorisé à poursuivre en année supérieure.</p>
                                </div>
                              </div>
                            }

                            <!-- INFO: REFUSÉE -->
                            @if (derog.statut === 'REFUSEE') {
                              <div class="info-box danger mt-3">
                                <i class="bi bi-x-circle-fill text-danger"></i>
                                <div>
                                  <strong>Dérogation refusée</strong>
                                  @if (derog.commentaireAdmin) {
                                    <p class="mb-0 mt-1 small">Motif : {{ derog.commentaireAdmin }}</p>
                                  }
                                </div>
                              </div>
                            }
  ...
  styles: [`
    ...
    .btn-refresh { padding: 0.75rem 1.25rem; background: white; color: #d97706; border: none; border-radius: 10px; font-weight: 600; cursor: pointer; display: flex; gap: 0.5rem; align-items: center; transition: 0.2s; }
    ...
    .data-table tbody tr.clickable:hover { background: #fffbeb; cursor: pointer; }
    .data-table tbody tr.expanded-row { background: #fffbeb; border-left: 4px solid #f59e0b; }
    ...
    .expand-icon.rotated { transform: rotate(180deg); color: #f59e0b; }
    ...
    .decision-form { background: white; padding: 1rem; border-radius: 8px; border: 1px solid #fecaca; margin-top: 1rem; }
...
export class DerogationManagementComponent implements OnInit {
  ...
  confirmRefusal(id: number, e: Event) {
    ...
    this.derogationService.refuserDerogation(id, this.authService.currentUser()!.id, this.refusalReason).subscribe({
      next: () => {
        this.showToast('Dérogation refusée', 'success');
        this.loadDerogations();
        this.expandedId.set(null);
      },
